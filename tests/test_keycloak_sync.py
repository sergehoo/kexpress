...
def _make_fake(calls):
    ...
    def fake_urlopen(req, timeout=None, context=None):
        ...
        if "/users" in url and method == "GET" and "email=" in url:
            return _FakeResp(200, [])  # aucun compte existant
...
def ctx(db):
    ...
    return dict(sub=sub, admin=admin, target=target, client=client)
...
def test_apply_sync_creates_user_and_records(ctx, monkeypatch):
    ...
    assert any(m == "POST" and url.rstrip("/").endswith("/users") for m, url in calls)
    assert any("/role-mappings/realm" in url for _m, url in calls)
